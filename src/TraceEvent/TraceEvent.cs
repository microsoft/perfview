    ...
    public unsafe abstract class TraceEvent
    {
        ...
        public string Dump(bool includePrettyPrint = false, bool truncateDump = false)
        {
            ...
            if (ActivityID != Guid.Empty || RelatedActivityID != Guid.Empty)
            {
                ...
                {
                    XmlAttrib(sb, "ActivityID", ActivityComputer.ActivityPathString(ActivityID));
                    if (ActivityComputer.IsActivityPath(ActivityID))        // Also print out the raw GUID
                        XmlAttrib(sb, "RawActivityID", ActivityID);
                }
                if (RelatedActivityID != Guid.Empty)
                    XmlAttrib(sb, "RelatedActivityID", ActivityComputer.ActivityPathString(RelatedActivityID));
                sb.AppendLine().Append(" ");
            }
#endif
