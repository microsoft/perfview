...
namespace Microsoft.Diagnostics.Tracing.Stacks
{
    ...
    public class MutableTraceEventStackSource : TraceEventStackSource
    {
        ...
        /// 
        /// This is a convenience routine for GetCallStack(callStackIndex, data.Thread), but is more efficient because the thread is only needed
        /// when the callstack index is Invalid and fetching the thread is non-trivial work.  
        /// </summary>  
        public virtual StackSourceCallStackIndex GetCallStack(CallStackIndex callStackIndex, TraceEvent data)
        {
            if (callStackIndex == CallStackIndex.Invalid)
            {
                if (data == null)
        ...
        /// stack (above the stack, where the thread and process woudl normally go) as 'top'.  If callStckMap is non-null 
    ...
    /// <summary>
    /// A mutable TraceEventStackSource that generates stacks based on activity tracing information in TraceLog
    /// Thus if a thread T1 executing activity A schedules a work item (activity B) on another thread T2, then
    /// all event on T2 while executing activity B are made children of the stack where Thread T1 scheduled B.  
    /// It is as if A called B.   
    /// </summary>
    public class ActivityAwareTraceEventStackSource : MutableTraceEventStackSource
    {
        /// <summary>
        /// Creates an instance of this class
        /// </summary>
        public ActivityAwareTraceEventStackSource(TraceLog log)
            : base(log)
        { }

        /// <summary>
        /// Returns a regular (non-activity-aware) callstack given the 'callStackIndex' and 'data' passed in
        /// Generally you don't need this method...
        /// </summary>
        public StackSourceCallStackIndex GetRawCallStack(CallStackIndex callStackIndex, TraceEvent data)
        {
            return base.GetCallStack(callStackIndex, data);
        }

        #region Implementation of MutableTraceEventStackSource
        /// <summary>
        /// Implements MutableTraceEventStackSource protocol
        /// </summary>
        public override StackSourceCallStackIndex GetCallStack(CallStackIndex callStackIndex, TraceEvent data)
        {
            if (data == null)
                return base.GetCallStack(callStackIndex, data);
            else
                return GetOrCreateActivityAwareCallStack(callStackIndex, data.Thread(), data.TimeStampQPC);
        }
        #endregion Implementation of MutableTraceEventStackSource

        #region Private 
        // -- Support methods for GetCallStack
        private StackSourceCallStackIndex GetOrCreateActivityAwareCallStack(CallStackIndex callStackIndex, TraceThread thread, long timeQPC, 
                                                ActivityIndex defCurrActIndex = ActivityIndex.Invalid)
        {
            if (thread == null)
                return StackSourceCallStackIndex.Invalid;

            Debug.Assert(defCurrActIndex == ActivityIndex.Invalid || defCurrActIndex == thread.GetActivityIndex(timeQPC));
            var currActIndex = defCurrActIndex;
            if (currActIndex == ActivityIndex.Invalid)
            {
                currActIndex = thread.GetActivityIndex(timeQPC);
                // if there isn't a current activity on the thread treat this as a regular stack
                if (currActIndex == thread.DefaultActivityIndex)
                    return GetCallStackThread(callStackIndex, thread);
            }

            StackSourceCallStackIndex cached;
            if (TryGetStackIndexMap(currActIndex, callStackIndex, out cached))
                return cached;

            TraceActivity currActivity = TraceLog.Activities[currActIndex];
            // if the current activity wasn't "scheduled" (e.g. the "Scheduling" event was lost) 
            // treat this as a regular stack
            Debug.Assert(currActivity != null);
            if (currActivity.creatorIndex == ActivityIndex.Invalid || currActivity.Creator == null)
                return GetCallStackThread(callStackIndex, thread);

            Debug.Assert(currActIndex != currActivity.creatorIndex);
            TraceActivity callerActivity = currActivity.Creator;

            CallStackIndex creationStack = TrimETWFrames(currActivity.CreationCallStackIndex);

            // get the stack for the "scheduled activity" event
            StackSourceCallStackIndex baseStack = GetOrCreateActivityAwareCallStack(creationStack, callerActivity.Thread, currActivity.creationTimeQPC, currActivity.creatorIndex);
            

            // stitch the "scheduled" stack and the current stack together
            return ConvertToStackSourceStack(baseStack, thread, callStackIndex, currActIndex);
        }

        /// <summary>
        /// Trims off frames that call ETW logic an return.   If the pattern is not matched, we return  callStackIndex
        /// </summary>
        private CallStackIndex TrimETWFrames(CallStackIndex callStackIndex)
        {
            ...
            {
                CodeAddressIndex codeAddressIndex = TraceLog.CallStacks.CodeAddressIndex(callStackIndex);
                MethodIndex methodIndex = TraceLog.CallStacks.CodeAddresses.MethodIndex(codeAddressIndex);

                callStackIndex = TraceLog.CallStacks.Caller(callStackIndex);
        ...
        /// <summary>
        /// create a new stack that is made up (from the root to leaf) first by all the frames in baseStack followed 
        /// by a "Task Executing on Thread 'thread'" node, then by all the frames of 'tail' excluding the thread
        /// pool logic near the root of 'tail'
        /// </summary>
        private StackSourceCallStackIndex ConvertToStackSourceStack(StackSourceCallStackIndex baseStack, TraceThread thread, CallStackIndex tail, ActivityIndex uaiIndex)
        {
            if (baseStack == StackSourceCallStackIndex.Invalid)
                return GetCallStackThread(tail, thread);
            else
                return GetThreadPoolCallStack(baseStack, thread, tail, uaiIndex);
        }

        /// <summary>
        /// create a stack in 'stackSource' that, starting from the root, consists of all the frames in BaseStack 
        /// followed by a "Task 'task' Executing on Thread 'thread'" node, then by all the frames of 'tail' excluding the 
        /// thread pool logic near the root of 'tail'
        /// </summary>
        private StackSourceCallStackIndex GetThreadPoolCallStack(StackSourceCallStackIndex baseStack, TraceThread thread, CallStackIndex tail, ActivityIndex uaiIndex)
        {
            Debug.Assert(baseStack != StackSourceCallStackIndex.Invalid);
            Debug.Assert(uaiIndex != ActivityIndex.Invalid);
            Debug.Assert(m_perActivityStackIndexMaps == null || !m_perActivityStackIndexMaps.ContainsKey(uaiIndex) || !m_perActivityStackIndexMaps[uaiIndex].ContainsKey(tail));
            ...
            var threadPoolTransition = FindThreadPoolTransition(tail);

            StackSourceFrameIndex taskMarkerFrame = IsRecursiveTask(tail, threadPoolTransition, baseStack);
            if (taskMarkerFrame != StackSourceFrameIndex.Invalid)
            {
                UpdateTaskMarkerFrame(taskMarkerFrame, thread.ThreadID.ToString());         // Add the thread ID to the 'STARTING TASK' frame if necessary
                return baseStack;
            }

            // var label = this.TraceLog.Activities[uaiIndex].Name;       
            StackSourceFrameIndex threadFrameIndex = this.Interner.FrameIntern("STARTING TASK on Thread " + thread.ThreadID);
            baseStack = this.Interner.CallStackIntern(threadFrameIndex, baseStack);

            var ret = SpliceStack(tail, threadPoolTransition, baseStack, uaiIndex);
            return ret;
        }

        // If the stack from 'start' (closest to execution) through 'stop'
        private StackSourceFrameIndex IsRecursiveTask(CallStackIndex startStack, CallStackIndex stopStack, StackSourceCallStackIndex baseStack)
        {
            ...
            for (; ; )
            {
                ...
                var existingFrameName = GetFrameName(GetFrameIndex(existingStacks), true);
                var newFrameCodeAddressIndex = TraceLog.CallStacks.CodeAddressIndex(newStacks);
                if (newFrameCodeAddressIndex == CodeAddressIndex.Invalid)
                    return StackSourceFrameIndex.Invalid;

                var newFrameMethodIndex = TraceLog.CodeAddresses.MethodIndex(newFrameCodeAddressIndex);
                if (newFrameMethodIndex == MethodIndex.Invalid)
                    return StackSourceFrameIndex.Invalid;
                var newFrameName = TraceLog.CodeAddresses.Methods.FullMethodName(newFrameMethodIndex);

                // TODO FIX NOW do this better. 
                if (!existingFrameName.EndsWith(newFrameName))
                    return StackSourceFrameIndex.Invalid;

                existingStacks = GetCallerIndex(existingStacks);
                newStacks = TraceLog.CallStacks.Caller(newStacks);
            }

            var frameIdx = GetFrameIndex(existingStacks);
            var frameName = GetFrameName(frameIdx, true);
        ...
        {
            var frameName = GetFrameName(taskMarkerFrame, true);
            ...
            Interner.UpdateFrameName(taskMarkerFrame, frameName + " " + newTaskID);
        }
        ...
        /// </summary>
        private StackSourceCallStackIndex SpliceStack(CallStackIndex startStack, CallStackIndex stopStack, StackSourceCallStackIndex baseStack, ActivityIndex uaiIndex)
        {
            if (startStack == CallStackIndex.Invalid || startStack == stopStack)
                return baseStack;

            var codeAddress = TraceLog.CallStacks.CodeAddressIndex(startStack);
            var caller = TraceLog.CallStacks.Caller(startStack);
            var callerStack = SpliceStack(caller, stopStack, baseStack, uaiIndex);
            var frameIdx = GetFrameIndex(codeAddress);
            StackSourceCallStackIndex result = Interner.CallStackIntern(frameIdx, callerStack);
            AddCallStackIndexMap(uaiIndex, startStack, result);
            return result;
        }
        ...
        private CallStackIndex FindThreadPoolTransition(CallStackIndex callStackIndex)
        {
            ...
            while (callStackIndex != CallStackIndex.Invalid)
            {
                ...
                // TODO FIX NOW fix if you don't have symbols 
                if ((uint)methodIndex < (uint)m_methodFlags.Length && (m_methodFlags[(int)methodIndex] & MethodFlags.TaskRun) != 0)
        ...
        private void ResolveWellKnownSymbols()
            ...
            TraceMethods methods = TraceLog.CodeAddresses.Methods;
            ...
            for (MethodIndex methodIndex = 0; methodIndex < (MethodIndex)methods.Count; methodIndex++)
                        {
                TraceModuleFile moduleFile = TraceLog.ModuleFiles[methods.MethodModuleFileIndex(methodIndex)];
                if (moduleFile == null)
                    continue;
        ...
        enum MethodFlags : byte
        {
            ...
            TaskSchedule = 2,             // This is a method that marks the scheduling of a task (frames toward execution are irrelevant)
            TaskScheduleHelper = 4,       // This method if 'above' (toward thread root), from a TaskSchedule should also be removed.  
        }

        // -- Optimization / caching helpers

        private void AddCallStackIndexMap(ActivityIndex uaiIndex, CallStackIndex csi, StackSourceCallStackIndex sscsi)
        {
            if (m_perActivityStackIndexMaps == null)
                m_perActivityStackIndexMaps = new Dictionary<ActivityIndex, Dictionary<CallStackIndex, StackSourceCallStackIndex>>(256);
            if (!m_perActivityStackIndexMaps.ContainsKey(uaiIndex))
                m_perActivityStackIndexMaps[uaiIndex] = new Dictionary<CallStackIndex, StackSourceCallStackIndex>();
            StackSourceCallStackIndex prevSscsi;
            Debug.Assert(!m_perActivityStackIndexMaps[uaiIndex].TryGetValue(csi, out prevSscsi) || prevSscsi == sscsi);
            m_perActivityStackIndexMaps[uaiIndex][csi] = sscsi;
        }

        private bool TryGetStackIndexMap(ActivityIndex uaiIndex, CallStackIndex csi, out StackSourceCallStackIndex sscsi)
        {
            sscsi = StackSourceCallStackIndex.Invalid;
            return (m_perActivityStackIndexMaps != null &&
                    m_perActivityStackIndexMaps.ContainsKey(uaiIndex) &&
                    m_perActivityStackIndexMaps[uaiIndex].TryGetValue(csi, out sscsi));
        }
        ...
        /// </summary>
        private Dictionary<ActivityIndex, Dictionary<CallStackIndex, StackSourceCallStackIndex>> m_perActivityStackIndexMaps;
        #endregion
    }
}
