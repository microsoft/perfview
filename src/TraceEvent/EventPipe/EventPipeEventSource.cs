...
namespace Microsoft.Diagnostics.Tracing.EventPipe
{
    public unsafe class EventPipeEventSource : TraceEventDispatcher
    {
        ...
        private readonly int _minimumReaderVersion;
        private readonly Dictionary<StreamLabel, EventMetadata> _eventMetadataDictionary = new Dictionary<StreamLabel, EventMetadata>();
        private readonly EventPipeTraceEventParser _eventParser;

        private TraceEventNativeMethods.EVENT_RECORD* _header;
        private readonly StreamLabel _endOfEventStream;

        private long _lastTimestamp = 0;
        private string _processName;
        private int _processId;

        internal EventPipeEventSource(Deserializer deserializer, EventPipeEventSourceInitialData data)
        {
            _deserializer = deserializer ?? throw new ArgumentNullException(nameof(deserializer));

            _version = data.Version;
            _minimumReaderVersion = data.ReaderVersion;
            ...
            _syncTimeUTC = data.CreationTime;
            sessionStartTimeQPC = data.StartTimeStamp;
            _syncTimeQPC = sessionStartTimeQPC;
            _QPCFreq = data.ClockFrequency;

            _endOfEventStream = data.EndOfStream;
