...
namespace Stats
{
    internal static class GcStats
    {
        ...
        private static string[][] CondemnedReasonsHtmlHeader = new string[(int)CondemnedReasonGroup.Max][]
        {
            ...
            new string[] {"Avoid_unproductive", "This happens when the GC detects previous attempts to do a full compacting GC is not making progress and therefore reduce its generation"},
            new string[] {"Pm_induced_fullgc_p", "This happens when a full gc is induced under provisional mode"},
            new string[] {"Pm_alloc_loh", "This happens when a large object heap allocation is requested under provisional mode"},
            new string[] {"Last_gen2_fragmented", "This happens when we had a high memory and high fragmentation detected after the last full blocking GC, indicating we have lot of pinned objects in gen 2, so reducing its generation"},
            new string[] {"Limit_before_oom", "This happens when the last gc was oom"},
            new string[] {"Limit_loh_frag", "This happens when we had a heap limit and the fragmentation is reaching 1/8 of it"},
            new string[] {"Limit_loh_reclaim", "This happens when we had a heap limit and we could reclaim 1/8 of it"},
            new string[] {"Servo_initial", "This happens when the servo tuning is trying to get some initial data"},
            new string[] {"Servo_ngc", "This happens when the servo tuning decides a blocking gc is appropriate"},
            new string[] {"Servo_bgc", "This happens when the servo tuning decides a background gc is appropriate"},
            new string[] {"Servo_postpone", "This happens when the servo tuning decides a gen2 gc should be postponed"},
            new string[] {"Stress_mix", "This happens in GCStress mix mode, every 10th GC is gen2"},
