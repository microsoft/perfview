...
namespace PerfViewExtensibility
{
    ...
    public class CommandEnvironment
    {
        ...
        public static void OpenStackViewer(Stacks stacks, Action<StackWindow> OnOpened = null)
        {
            GuiApp.MainWindow.Dispatcher.BeginInvoke((Action)delegate()
            {
                ...
                else
                {
                    ...
                    var file = PerfViewFile.Get(filePath) as XmlPerfViewFile;
                    if (file == null)
            ...
            Action<string, TextWriter, WebBrowserWindow> DoCommand = null, Action<WebBrowserWindow> OnOpened = null)
        {
            GuiApp.MainWindow.Dispatcher.BeginInvoke((Action)delegate()
            {
                ...
                viewer.Height = 600;
                viewer.Title = title;
                viewer.Browser.Navigate(Path.GetFullPath(htmlFilePath));
                viewer.Show();
                if (OnOpened != null)
        ...
        {
            LogFile.WriteLine("[Opening Excel on {0}]", csvFilePath);
            Command.Run("excel " + Command.Quote(csvFilePath), new CommandOptions().AddStart().AddTimeout(CommandOptions.Infinite));
        }
        /// <summary>
    ...
    public class Events : ETWEventSource
    {
        ...
        void OpenInExcel()
        {
            ...
            SaveAsCSV(csvFile);
            log.WriteLine("Launching Excel.");
            Command.Run("excel " + Command.Quote(csvFile), new CommandOptions().AddStart().AddTimeout(CommandOptions.Infinite));
            log.WriteLine("Excel launched.");
    ...
    internal static class Extensions
    {
        ...
        public static IEnumerable<string> GetExtensionDlls()
        {
            ...
            return Directory.GetFiles(ExtensionsDirectory, "*.dll", SearchOption.TopDirectoryOnly);
        }
            ...
            if (instance == null)
            {
                ...
                if (!LoadedObjects.TryGetValue(fileSpec, out instance))
                {
                    ...
                    if (!File.Exists(fullFilePath))
                        throw new FileNotFoundException("Could not find file " + fullFilePath, fullFilePath);
...
namespace PerfViewExtensibility
{
    ...
    public class Commands : CommandEnvironment
    {
        ...
        /// </summary>
        /// <param name="eventSourceName"> The list of event sources</param>
        public void Listen(string eventSourceName)
        {
            ...
            using (var session = new TraceEventSession(sessionName))
            {
                ...
                GuiApp.MainWindow.Dispatcher.BeginInvoke((Action)delegate()
                {
                    ...
                    logTextWindow.Title = "Listening to " + eventSourceName;
                    logTextWindow.Show();
                });
                ...
                foreach (string providerName in eventSourceName.Split(','))
                {
        ...
        public void DirectorySize(string directoryPath = ".", string outputFileName=null)
        {
            if (outputFileName == null)
            ...
            FileSizeStackSource fileSizeStackSource = new FileSizeStackSource(directoryPath);
        ...
        public void AppFabricCacheRequestsToCsv(string etlFileNamePattern, string csvOutputFileName = null, bool launchExcel = true)
        {
            ...
            if (launchExcel)
            {
                LogFile.WriteLine("[Launching Excel on {0}]", csvOutputFileName);
                Command.Run("excel " + Command.Quote(csvOutputFileName), new CommandOptions().AddStart().AddTimeout(CommandOptions.Infinite));
            }
        }
#endif
        ...
        public void CapGCStats(string etlFile)
        {
            ...
            TraceEventDispatcher eventDispatcher = source.Events.GetSource();

            KernelTraceEventParser kernel = source.Kernel;

            eventDispatcher.Kernel.SystemConfigCPU += delegate(SystemConfigCPUTraceData data)
            {
            ...
            eventDispatcher.Kernel.SysConfigBuildInfo += delegate(BuildInfoTraceData data)
            {
                report.OSInfo.Name = data.ProductName;
                report.OSInfo.Build = data.BuildLab;
            };

            var gcStats = Stats.GCProcess.Collect(eventDispatcher, 1);
            ...
            // Dump the EventStats into the report.
            report.EventStats.PopulateEventCounts(source.Stats);

            //remove "etl" and "zip" extension
            string outputFileName = Path.ChangeExtension(savedEtlFile, null);
            if (Path.GetExtension(outputFileName).CompareTo(".etl") == 0)   //in case extension was .etl.zip, remove both
                outputFileName = Path.ChangeExtension(outputFileName, null);

            ClrCap.CAP.GenerateCAPReport(gcStats, report);

            report.WriteToFileXML(outputFileName);
        }
#endif
        ...
        private Dictionary<string, ScenarioConfig> DeserializeScenarioConfig(XmlReader reader, XmlWriter output, TextWriter log, string baseDir)
        {
            ...
            do
            {
                ...
                double startTime =
                    (startTimeText != null) ?
                        double.Parse(startTimeText, CultureInfo.InvariantCulture) :
                        double.NegativeInfinity;

                double endTime =
                    (endTimeText != null) ?
                        double.Parse(endTimeText, CultureInfo.InvariantCulture) :
                ...
                foreach (string file in Directory.GetFiles(dir, pattern))
                {
                    ...
                    if (processPattern != null)
                    {
                        ...
                        if (match.Groups.Count < 1)
                            match = defaultRegex.Match(PerfViewFile.GetBaseName(file));
...
public static class TraceEventStackSourceExtensions
{
    ...
    public static MutableTraceEventStackSource BlockedTimeStacks(this TraceLog eventLog, TraceProcess process = null, bool showUnknownAddresses = false)
    {
        ...
        var computer = new ETLPerfViewData.ThreadTimeComputer(eventLog);
    ...
    public static MutableTraceEventStackSource ThreadTimeAspNetWithTasksStacks(this TraceLog eventLog, TraceProcess process = null, bool showUnknownAddresses = false)
    {
        // Use MutableTraceEventStackSource to disable activity tracing support
        var stackSource = new MutableTraceEventStackSource(eventLog);
        stackSource.ShowUnknownAddresses = App.CommandLineArgs.ShowUnknownAddresses;
