...
namespace PerfView
{
            ...
            if (Viewer == null)
            {
                ...
                worker.StartWork("Opening " + Name, delegate()
                {
                    ...
                    worker.EndWork(delegate()
                    {
                        ...
                        Viewer.Height = 600;
                        Viewer.Title = Title;
                        Viewer.Browser.Navigate(reportFileName);
                        Viewer.Show();
                        if (doAfter != null)
                            doAfter();
                    });
    ...
    public partial class ETLPerfViewData : PerfViewFile
    {
        ...
        protected internal override StackSource OpenStackSourceImpl(string streamName, TextWriter log, double startRelativeMSec = 0, double endRelativeMSec = double.PositiveInfinity, Predicate<TraceEvent> predicate = null)
        {
            ...
            {
                // Used to speed things up for the most common pseudo-frames.  
                ...
                bool processed = false;         // Has this event already been processed by another callback. 
                ...
                // This is the main routine, and we set it up here because we use it both
                // on the 'everyEvent' as well as the start-stop callback 
                Action<TraceEvent> logEvent = delegate(TraceEvent data)
                {
                    // We only want to do this once per event.  We may however call this from the start-stop callback.  
                    var hasBeenProcessed = processed;
                    processed = false;
                    if (hasBeenProcessed)
                    {
                        Debug.Assert(activityComputer != null);
                        activityComputer.ClearCache();
                        return;
                    }
                    ...
                    for (int i = 0; i < fieldNames.Length; i++)
                    {
                        ...
                        if (fieldName.IndexOf("Name", StringComparison.OrdinalIgnoreCase) >= 0 ||
                            fieldName == "OpenPath" || fieldName == "Url" || fieldName == "ConnectionId" || fieldName == "message" ||
                            fieldName == "ExceptionType" || fieldName == "ExceptionMessage")
                        {
                ...
                if (isAnyTaskTree || isAnyWithTasks || isAnyWithStartStop)
                {
                    ...
                    if (isAnyWithStartStop)
                    {
                        ...
                        startStopComputer.OnStartOrStop += delegate(TraceEvent data)
                        {
                            // This is a bit of a hack.   Because activity's are sometimes wrong, every time we get a change in the
                            // Start-stops we clear the cache 
                            activityComputer.ClearCache();

                            // We do logging of the event here because in the case of a stop, we need the logging to happen 
                            // BEFORE we update the start-stop state (otherwise the stop does not look like it is in the activity).
                            // THe OnStartStop callback, is carefully speced to let this happen.  
                            // We have logic in logEvent so that when the 'EveryEvent' callback is then made we recognize that 
                            // we have already logged the event and not do it twice.   
                            logEvent(data);
                            processed = true;
                        };
                    }
                ...
                // It is important that this subscription happen AFTER the subscriptions in the StartStopActivityComputer, because
                // we want that computer's state to be updated BEFORE we ask it for what that stacks are.  
                eventSource.AllEvents += logEvent;
            ...
            else if (streamName == "Net Virtual Alloc")
            {
                ...
                eventSource.Kernel.AddCallbackForEvents<VirtualAllocTraceData>(delegate(VirtualAllocTraceData data)
                {
                        ...
                        VirtualAllocTraceData.VirtualAllocFlags.MEM_RELEASE)) != 0)
                    {
                        ...
                        var process = data.Log().Processes.GetProcess(data.ProcessID, data.TimeStampRelativeMSec + 1000);
                        if (process == null)
                        {
                            droppedEvents++;
                            return;
                        ...
                        var processIndex = process.ProcessIndex;
                        ...
                        if ((data.Flags & VirtualAllocTraceData.VirtualAllocFlags.MEM_COMMIT) != 0)
                        {
                            ...
                            // But do count them.  
                            if (data.TimeStampRelativeMSec >= process.StartTimeRelativeMsec)
        ...
        protected internal override void ConfigureStackWindow(string stackSourceName, StackWindow stackWindow)
        {
            ...
            if (stackSourceName == "CPU" || stackSourceName == "Blocked Time" ||
        ...
        protected override Action<Action> OpenImpl(Window parentWindow, StatusBar worker)
        {
            ...
            if (hasCSwitchStacks && AppLog.InternalUser)
                m_Children.Add(new PerfViewStackSource(this, "Blocked Time"));
    ...
    public class ETWEventSource : EventSource
    {
        ...
        public override IEnumerable<EventRecord> Events
        {
            get
            {
                ...
                {
                    ETWEventRecord emptyEventRecord = new ETWEventRecord();
                    var startStopRecords = new Dictionary<StartStopKey, double>(10);

                    EventVisitedVersion.CurrentVersion++;
                    foreach (var data in m_tracelog.Events.FilterByTime(startTime, endTime))
                    {
                        ...
                        if (textFilter != null)
                        {
                            eventRecord = new ETWEventRecord(data, columnOrder, NonRestFields, durationMSec);
        ...
        internal class ETWEventRecord : EventRecord
        {
            ...
            internal ETWEventRecord() : base(0) { }

            internal ETWEventRecord(TraceEvent data, Dictionary<string, int> columnOrder, int nonRestFields, double durationMSec)
                : base(nonRestFields)
            {
    ...
    class PdbScopePerfViewFile : PerfViewFile
    {
        ...
        protected internal override void ConfigureStackWindow(string stackSourceName, StackWindow stackWindow)
        {
            ...
            stackWindow.GroupRegExTextBox.Items.Add("[group modules]           {%}!->module $1;EEType->module CoreLib;InterfaceDispatchCell->module CoreFX");
