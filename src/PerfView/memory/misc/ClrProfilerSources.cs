    ...
    public class ClrProfilerAllocStackSource : StackSource
    {
        ...
        public override void ProduceSamples(Action<StackSourceSample> callback)
        {
            ...
            for (int i = 0; i < allocs.Count; i++)
            {
                ...
                sample.TimeRelMSec = allocs[i].MsecFromStart;

                // TODO this is for debugging can remove
                // var stackId = m_clrProfiler.GetAllocStack(Allocs[i].AllocId);
                // var clrProfilerStr = new ProfilerStack(m_clrProfiler, stackId).ToString();
        ...
        public override StackSourceCallStackIndex GetCallerIndex(StackSourceCallStackIndex callStackIndex)
        {
            ...
            {
                // It is a allocation index, so the frame name is the nodeId name (offset by the number of methods)
        ...
        public override StackSourceSample GetSampleByIndex(StackSourceSampleIndex sampleIndex)
        {
            ...
            m_sample.TimeRelMSec = allocs[(int)sampleIndex].MsecFromStart;
            return m_sample;
        }
