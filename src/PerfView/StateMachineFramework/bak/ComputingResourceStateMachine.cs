...
namespace PerfView
{
    ...
    public sealed class ComputingResourceStateMachine
    {
        public ComputingResourceStateMachine(MutableTraceEventStackSource outputStackSource, ScenarioConfiguration configuration, ComputingResourceViewType viewType)
        {
            ...
            m_activityComputer = new ActivityComputer(configuration.TraceLog, outputStackSource);
        ...
        #region private
        // callbacks for particular ETW events
        ...
        private void OnGCAllocationTick(GCAllocationTickTraceData data)
        {
            ...
            StackSourceCallStackIndex callStackIndex = GetCallStack(data, thread);
        ...
        internal StackSourceCallStackIndex GetCallStack(TraceEvent data, TraceThread thread)
        {
            StackSourceCallStackIndex callStackIndex = m_Configuration.GetCallStackUpToThread(m_OutputStackSource, data);
    ...
    public abstract class ScenarioConfiguration
    {
        ...
        /// <summary>
        /// Return a call stack associated with 'outputStackSource' that is just the part close to the root of the stack (process, request frames).  
        /// </summary>
        public abstract StackSourceCallStackIndex GetCallStackUpToThread(MutableTraceEventStackSource outputStackSource, TraceEvent data);
    ...
    {
        /// <summary>
        /// Get the 'logical' call stack from PROCESS ROOT (the root of all stacks) to (but not including) the frame for the
        /// thread.   By default (if you can't attribute it to anything else) it will just be attributed to the process, however
        /// it is likely that you want to insert pseudo-frames for the request and other logical groupings here.  
        /// 
        /// The actual method frames within a thread, as well as any resource specific pseudo-frames (e.g. BLOCKING, ...)
        ...
        public virtual StackSourceCallStackIndex GetCallStackIndex(MutableTraceEventStackSource stackSource, TraceThread thread, TraceEvent data)
        {
            ...
            StackSourceFrameIndex requestsFrameIndex = stackSource.Interner.FrameIntern("Unattributed");
    ...
    internal sealed class ComputingResourceThreadState
    {
            ...
            TraceEvent data)
        {
            ...
            if (ThreadBlocked)
            {
                ...
                StackSourceCallStackIndex callStackIndex = stateMachine.GetCallStack(data, thread);

                // Add Pseudo-frames representing the kind of resource.  
