...
public class CommandLineParser
{
    ...
    public static void ParseForConsoleApplication(Action<CommandLineParser> parseBody)
    {
        try
        {
            ...
            // TODO review this.  IT is too subtle.  
            if (parser.args == null)
                parser.ParseWords();
            if (parser.args.Count > 0 && (parser.args[0] == "/?" || parser.args[0] == "-?"))
                help = true;
            if (parser.args.Count > 1 && (parser.args[1] == "/?" || parser.args[1] == "-?"))
                help = true;
            if (help)
            {
            ...
            if (help)
            {
                ...
                {
                    parameterSet = ((parser.args[0] == "/?" || parser.args[0] == "-?") ? parser.args[1] : parser.args[0]);
                ...
                if (helpString.Length == 0)
                    helpString = parser.GetHelp(maxLineWidth, parameterSet);
                ShowHelp(helpString);
    ...
    /// </summary>
    public string GetHelp(int maxLineWidth, string pattern)
    {
        ...
        foreach (CommandLineParameter parameter in parameterDescriptions)
        {
            if (parameter.IsParameterSet && parameter.Name.Length != 0 &&
                (pattern == null || parameter.Name.IndexOf(pattern, StringComparison.OrdinalIgnoreCase) >= 0))
            {
                sb.AppendLine().Append('-', maxLineWidth - 1).AppendLine();
    ...
    private void ParseWords()
    {
        ...
        ParseWordsWorker(commandLine);
    }

    private void ParseWordsWorker(string commandLineOrResponse)
    {
        // TODO review this carefully.
        int wordStartIndex = -1;
        bool hasExcapedQuotes = false;
        int numQuotes = 0;
        for (int i = 0; i < commandLineOrResponse.Length; i++)
        {
            char c = commandLineOrResponse[i];
            if (c == '"')
            {
                numQuotes++;
                if (wordStartIndex < 0)
                ...
                for (; ; )
                {
                    if (i >= commandLineOrResponse.Length)
                        throw new CommandLineParserException("Unmatched quote at position " + i + ".");
                    c = commandLineOrResponse[i];
                    if (c == '"')
                    {
                        if (i > 0 && commandLineOrResponse[i - 1] == '\\')
            ...
            // new line characters, coming from response files, when outside quoted strings, 
            // are treated as word separators. inside quoted strings they're item separators
            else if (c == ' ' || c == '\t' || c == '\n' || c == '\r')
            {
                if (wordStartIndex >= 0)
                {
                    AddWord(commandLineOrResponse, wordStartIndex, i, numQuotes, hasExcapedQuotes);
                    wordStartIndex = -1;
        ...
        if (wordStartIndex > 0)
            AddWord(commandLineOrResponse, wordStartIndex, commandLineOrResponse.Length, numQuotes, hasExcapedQuotes);
    }

    private void AddWord(string commandLineOrResponse, int wordStartIndex, int wordEndIndex, int numQuotes, bool hasExcapedQuotes)
    {
        if (!readExe)
        {
            readExe = true;
        ...
        if (numQuotes > 0)
        {
            ...
            if (!hasExcapedQuotes && numQuotes == 1 && commandLineOrResponse[wordStartIndex] == '"' && commandLineOrResponse[wordEndIndex - 1] == '"')
                word = commandLineOrResponse.Substring(wordStartIndex + 1, wordEndIndex - wordStartIndex - 2);
            else
            {
                ...
                for (int i = wordStartIndex; i < wordEndIndex; )
                {
                    ...
                    if (c != '"')
                    {
                        if (c == '\\' && i < wordEndIndex && commandLineOrResponse[i] == '"')
                        {
                            sb.Append('"');
                            i++;
                        }
                        ...
                        {
                            // inside quoted strings translate \r, \n, \r\n into semi-colons
                            // (they're item separators)
                            if (c == '\r' || c == '\n')
                            {
                                c = ';';
                                if (i < wordEndIndex && (commandLineOrResponse[i] == '\r' || commandLineOrResponse[i] == '\n'))
                                    ++i;
                            }
                            sb.Append(c);
                        }
            ...
            word = commandLineOrResponse.Substring(wordStartIndex, wordEndIndex - wordStartIndex);
        
        // handle response files
        if (word.StartsWith("@"))
        {
            string responseFile = word.Substring(1);
            if (!System.IO.File.Exists(responseFile))
                throw new CommandLineParserException("Response file missing " + responseFile + ".");
            string response = System.IO.File.ReadAllText(responseFile);
            ParseWordsWorker(response);
        }
        else
        {
            args.Add(word);
    ...
    private int GetNextOccuranceQualifier(string name, string[] aliases)
    {
        ...
        if (match != null)
        {
            ...
            else
            {
                ...
                if (nextPos >= 0)
                    dashedParameterEncodedPositions[name] = SetMultiple(nextPos);
                else
    ...
    private static bool IsMultiple(int encodedPos) { return (encodedPos & MultiplePositions) != 0; }
    ...
    // As we scan positional parameters, this variable keeps track of where we are. 
