...
export class TreeNode extends React.PureComponent<Props, State> {
  ...
  static renderTreeNode(
    ...
    autoExpand: boolean,
    isCollapsed: boolean,
    toggleTreeNode: TreeNode["toggleTreeNode"],
    obj: TreeNode
  ) {
    return (
      <React.Fragment>
  ...
  render() {
    return TreeNode.renderTreeNode(
      this.props.routeKey,
      this.props.node,
      this.props.indent,
      this.props.callTreeNodeId,
      this.props.node.hasChildren ? this.state.children : [],
      this.props.node.hasChildren && this.state.children.length === 1,
