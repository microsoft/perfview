...
export class StackViewerFilter extends React.PureComponent<Props, State> {
  ...
  constructor(props: Props) {
    super(props);
    const data = JSON.parse(base64url.decode(this.props.routeKey, "utf8"));
    this.state = {
      symbolLog: "",
      symbolLookupStatus: "",
      minCount: 50,
      newRouteKey: this.props.routeKey,
      start: data.d,
      end: data.e,
      groupPats: data.f,
      foldPats: data.g,
  ...
  handleFoldPatsChange(e: React.ChangeEvent<HTMLInputElement>) {
  ...
  handleIncPatsChange(e: React.ChangeEvent<HTMLInputElement>) {
    this.setState({ incPats: e.target.value });
  }

  handleExcPatsChange(e: React.ChangeEvent<HTMLInputElement>) {
  ...
  handleOnClick(e: React.MouseEvent<HTMLButtonElement>) {
    ...
    const newRouteKeyJsonString = JSON.stringify({
      ...
      d: this.state.start,
      e: this.state.end || "",
      f: this.state.groupPats || "",
      g: this.state.foldPats || "",
      h: this.state.incPats || "",
      i: this.state.excPats || "",
      j: this.state.foldPct || "",
      k: this.state.drillIntoKey,
      l: oldRouteKey.l,
    });
  ...
  handleLookupWarmSymbolsMinCount(e: React.ChangeEvent<HTMLInputElement>) {
    this.setState({ minCount: parseInt(e.target.value, 10) });
  }
