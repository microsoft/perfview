...
namespace PerfViewExtensibility
{
    ...
    public class CommandEnvironment
    {
        ...
        ////                if (xmlFile == null)
        ////                    throw new Exception("Currently only support ETL files and XML files");
    ...
    public class ETLDataFile : DataFile
    {
        ...
        ////    if (moduleFiles.Count == 0)
        ////        throw new Exception("Could not find module " + simpleModuleName + " in trace.");
    ...
    ////                    }
    ////                Failed:
    ////                    throw new Exception("Error: " + errorMessage + "  '" + line + @"' line " + lineNum + @" in PerfViewExtensions\PerfViewStartup file");
    ////                }
    ////            }
    ////        }
    ////        catch (Exception e)
    ...
    ////            try
    ////            {
    ////                //// TODO: Figure out how to load assembly without using LoadFrom, since it does not exist in CoreFX
    ////                ////var assembly = Assembly.LoadFrom(extensionDllPath);
    ////                var assemblyName = new AssemblyName();
    ////                assemblyName.Name = extensionDllPath;
    ...
    ////        var assemblyName = method.DeclaringType.AssemblyQualifiedName;
    ////        if (string.Compare(assemblyName, "Global", StringComparison.OrdinalIgnoreCase) != 0)
    ...
    ////                var assemblyName = new AssemblyName();
    ////                assemblyName.Name = fullFilePath;
    ////                var assembly = Assembly.Load(assemblyName);

    ////                instanceType = assembly.GetType("Commands");
    ////                if (instanceType == null)
    ////                    throw new Exception("Could not find type 'Commands' in " + fullFilePath);
    ...
    ////            ////   null, instance, args);
    ////            var method = instanceType.GetTypeInfo().GetDeclaredMethod("methodSpec");
...
////                    Value = reader.ReadContentAsString().Trim();
////                }
////                else if (!reader.Read())
////                    break;
////            }
 ...
 namespace PerfViewExtensibility
{
    ...
    public class Commands : CommandEnvironment
    {
        ...
        public void SaveCPUStacks(string etlFileName, string processName = null)
        {
            using (var etlFile = OpenETLFile(etlFileName))
            {
                ...
                if (processName != null)
                {
                    ...
                    if (process == null)
                        throw new Exception("Could not find process named " + processName);
                }
        ...
        public void SaveScenarioCPUStacks(string scenario)
        {
            ...
            if (configs.Count == 0)
            {
                throw new Exception("No ETL files specified");
            }
            ...
            foreach (var configPair in configs)
            {
                ...
                if (processOfInterest == null & !wildCard)
                    throw new Exception("Process of interest could not be located for " + filename);
        ...
        public void WriteGCDumpAsXml(string gcDumpFileName, string outputFileName = null)
        {
            ...
            //// TODO: Figure out why XmlGcHeapDump is not found
            ////using (StreamWriter writer = File.CreateText(outputFileName))
        ...
        public void Listen(string etwProviderNames)
        {
            ...
            using (var session = new TraceEventSession(sessionName))
            {
                ...
                if (!App.CommandLineArgs.NoGui)
                {
                    //// TODO: Have yet to find CoreCLR equivalent to Dispatcher. Will probably strip all of this GUI stuff out, anyway.
                    ////GuiApp.MainWindow.Dispatcher.BeginInvoke((Action)delegate()
                    ////{
        ...
        public void DirectorySize(string directoryPath = null, string outputFileName = null)
        {
            ...
            File.OpenWrite(outputFileName).Dispose();
        ...
        public void ImageSize(string inputExeName = null, string outputFileName = null)
        {
            ...
            {
                if (App.CommandLineArgs.NoGui)
                    throw new Exception("Must specify an input EXE name");
                //// TODO: Have yet to find CoreCLR equivalent to Dispatcher. Will probably strip all of this GUI stuff out, anyway.
                // Hop to the GUI thread and get the arguments from a dialog box and then call myself again.  
                ////GuiApp.MainWindow.Dispatcher.BeginInvoke((Action)delegate()
                ////{
            ...
            if (!File.Exists(pdbScopeExe))
                throw new Exception(@"The PerfViewExtensions\PdbScope.exe file does not exit.   ImageSize report not possible");
            ...
            if (!File.Exists(outputFileName) || File.GetLastWriteTimeUtc(outputFileName) <= File.GetLastWriteTimeUtc(inputExeName))
            {
                ...
                if (!File.Exists(pdbScopeOutputFile))
                    throw new Exception("Error PdbScope did not create a file " + pdbScopeOutputFile);
        ...
        public void ListProviderKeywords(string providerNameOrGuid)
        {
            ...
            if (Regex.IsMatch(providerNameOrGuid, "........-....-....-....-............"))
            {
                if (!Guid.TryParse(providerNameOrGuid, out providerGuid))
                    throw new Exception("Could not parse Guid '" + providerNameOrGuid + "'");
            }
            ...
            else
            {
                ...
                if (providerGuid == Guid.Empty)
                    throw new Exception("Could not find provider name '" + providerNameOrGuid + "'");
            }
        ...
        public void ListProvidersInProcess(string processNameOrId)
        {
            ...
            if (processID < 0)
                throw new Exception("Could not find a process with a name or ID of '" + processNameOrId + "'");
        ...
        private Dictionary<string, ScenarioConfig> DeserializeScenarioConfig(XmlReader reader, XmlWriter output, TextWriter log, string baseDir)
        {
            ...
            if (!reader.ReadToDescendant("ScenarioConfig"))
                throw new Exception("The file does not have a Scenario element");

            if (!reader.ReadToDescendant("Scenarios"))
                throw new Exception("No scenarios specified");
            ...
            do
            {
                ...
                if (filePattern == null)
                    throw new Exception("File pattern is required.");

                if (!filePattern.EndsWith(".etl") && !filePattern.EndsWith(".etl.zip"))
                    throw new Exception("Files must be ETL files.");
