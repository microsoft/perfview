            ...
            out Zip64ExtraField zip64Block)
        {
            ...
            try
            {
                ...
                using (BinaryReader reader = new BinaryReader(ms))
                {
                    ...
                    if (zip64Block._uncompressedSize < 0) throw new InvalidDataException(System.IO.Compression.Messages.FieldTooBigUncompressedSize);
                    if (zip64Block._compressedSize < 0) throw new InvalidDataException(System.IO.Compression.Messages.FieldTooBigCompressedSize);
                    if (zip64Block._localHeaderOffset < 0) throw new InvalidDataException(System.IO.Compression.Messages.FieldTooBigLocalHeaderOffset);
                    if (zip64Block._startDiskNumber < 0) throw new InvalidDataException(System.IO.Compression.Messages.FieldTooBigStartDiskNumber);
