...
namespace Microsoft.Diagnostics.Runtime.Utilities
{
    ...
    public unsafe sealed class PEFile : IDisposable
    {
        ...
        private void Init(Stream stream, string filePath, bool virt)
        {
            ...
            Header = new PEHeader(m_headerBuff.Fetch(0, 1024), virt);
            ...
            if (Header.DebugDirectory.VirtualAddress != 0)
            {
                ...
                for (int i = 0; i < debugCount; i++)
                {
                    if (debugEntries[i].Type == IMAGE_DEBUG_TYPE.CODEVIEW)
                    {
                        ...
                        int ptr = m_virt ? (int)debugEntries[i].AddressOfRawData : (int)debugEntries[i].PointerToRawData;
    ...
    public unsafe sealed class PEHeader
    {
        ...
        /// </summary>
        public PEHeader(void* startOfPEFile, bool virt)
        {
            this.virt = virt;
    ...
    {
        public PEBuffer(Stream stream, int buffSize = 1024)
        {
            m_stream = stream;
            GetBuffer(buffSize);
        }
