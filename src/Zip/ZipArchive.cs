    ...
    public class ZipArchive : IDisposable {
        ...
        public ReadOnlyCollection<ZipArchiveEntry> Entries
        {
            get
            {
                ...
                if (_mode == ZipArchiveMode.Create)
                    throw new NotSupportedException(System.IO.Compression.Messages.EntriesInCreateMode);
        ...
        internal Encoding EntryNameEncoding
        {
            ...
            private set
            {
                ...
                {
                    throw new ArgumentException("Entry Name not Supported", "entryNameEncoding");
                }
        ...
        private ZipArchiveEntry DoCreateEntry(String entryName, CompressionLevel? compressionLevel)
        {
            ...
            if (String.IsNullOrEmpty(entryName))
                throw new ArgumentException(System.IO.Compression.Messages.CannotBeEmpty, "entryName");

            if (_mode == ZipArchiveMode.Read)
                throw new NotSupportedException(System.IO.Compression.Messages.CreateInReadMode);
        ...
        internal void AcquireArchiveStream(ZipArchiveEntry entry)
        {
            ...
            if (_archiveStreamOwner != null)
            {
                ...
                {
                    throw new IOException(System.IO.Compression.Messages.CreateModeCreateEntryWhileOpen);
                }
            }
        ...
        private void Init(Stream stream, ZipArchiveMode mode, Boolean leaveOpen)
        {
            ...
            try
            {
                ...
                switch (mode)
                {
                    case ZipArchiveMode.Create:
                        if (!stream.CanWrite)
                            throw new ArgumentException(System.IO.Compression.Messages.CreateModeCapabilities);
                        break;
                    case ZipArchiveMode.Read:
                        if (!stream.CanRead)
                            throw new ArgumentException(System.IO.Compression.Messages.ReadModeCapabilities);
                        if (!stream.CanSeek)
                        {
                    ...
                    case ZipArchiveMode.Update:
                        if (!stream.CanRead || !stream.CanWrite || !stream.CanSeek)
                            throw new ArgumentException(System.IO.Compression.Messages.UpdateModeCapabilities);
                        break;
                    default:
        ...
        private void ReadCentralDirectory()
        {
            try
            {
                ...
                if (numberOfEntries != _expectedNumberOfEntries)
                    throw new InvalidDataException(System.IO.Compression.Messages.NumEntriesWrong);
            }
            catch (EndOfStreamException ex)
            {
                throw new InvalidDataException(System.IO.Compression.Messages.CentralDirectoryInvalid, ex);
            }
        }
        ...
        private void ReadEndOfCentralDirectory()
        {
            try
            {
                ...
                if (!ZipHelper.SeekBackwardsToSignature(_archiveStream, ZipEndOfCentralDirectoryBlock.SignatureConstant))
                    throw new InvalidDataException(System.IO.Compression.Messages.EOCDNotFound);
                ...
                if (eocd.NumberOfThisDisk != eocd.NumberOfTheDiskWithTheStartOfTheCentralDirectory)
                    throw new InvalidDataException(System.IO.Compression.Messages.SplitSpanned);
                        ...
                        || eocd.NumberOfEntriesInTheCentralDirectory == ZipHelper.Mask16Bit)
                {
                    ...
                    if (ZipHelper.SeekBackwardsToSignature(_archiveStream, Zip64EndOfCentralDirectoryLocator.SignatureConstant))
                    {
                        ...
                        if (locator.OffsetOfZip64EOCD > (UInt64)Int64.MaxValue)
                            throw new InvalidDataException(System.IO.Compression.Messages.FieldTooBigOffsetToZip64EOCD);
                        ...
                        if (!Zip64EndOfCentralDirectoryRecord.TryReadBlock(_archiveReader, out record))
                            throw new InvalidDataException(System.IO.Compression.Messages.Zip64EOCDNotWhereExpected);

                        _numberOfThisDisk = record.NumberOfThisDisk;

                        if (record.NumberOfEntriesTotal > (UInt64)Int64.MaxValue)
                            throw new InvalidDataException(System.IO.Compression.Messages.FieldTooBigNumEntries);
                        if (record.OffsetOfCentralDirectory > (UInt64)Int64.MaxValue)
                            throw new InvalidDataException(System.IO.Compression.Messages.FieldTooBigOffsetToCD);
            ...
            {
                throw new InvalidDataException(System.IO.Compression.Messages.CDCorrupt, ex);
            }
