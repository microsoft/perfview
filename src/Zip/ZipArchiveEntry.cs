...
namespace System.IO.Compression
{
    ...
    public class ZipArchiveEntry
    {
        ...
        {           
            ...
            if (_storedEntryNameBytes.Length > UInt16.MaxValue)
                throw new ArgumentException(System.IO.Compression.Messages.EntryNamesTooLong);
        ...
        public Int64 CompressedLength
        {
            get
            {
                ...
                if (_everOpenedForWrite)
                    throw new InvalidOperationException(System.IO.Compression.Messages.LengthAfterWrite);
        ...
        public DateTimeOffset LastWriteTime
        {
            ...
            set
            {
                ...
                if (_archive.Mode == ZipArchiveMode.Read)
                    throw new NotSupportedException(System.IO.Compression.Messages.ReadOnlyArchive);
                if (_archive.Mode == ZipArchiveMode.Create && _everOpenedForWrite)
                    throw new IOException(System.IO.Compression.Messages.FrozenAfterWrite);
                if (value.DateTime.Year < ZipHelper.ValidZipDate_YearMin || value.DateTime.Year > ZipHelper.ValidZipDate_YearMax)
                    throw new ArgumentOutOfRangeException("value", System.IO.Compression.Messages.DateTimeOutOfRange);
        ...
        public void Delete()
        {
            ...
            if (_currentlyOpenForWrite)
                throw new IOException(System.IO.Compression.Messages.DeleteOpenEntry);

            if (_archive.Mode != ZipArchiveMode.Update)
                throw new NotSupportedException(System.IO.Compression.Messages.DeleteOnlyInUpdate);
        ...
        */
            ...
            get
            {
                if (_storedOffsetOfCompressedData == null)
                {
                    ...
                    if (!ZipLocalFileHeader.TrySkipBlock(_archive.ArchiveReader))
                        throw new InvalidDataException(System.IO.Compression.Messages.LocalFileHeaderCorrupt);
        ...
        private String DecodeEntryName(Byte[] entryNameBytes)
        {
            ...
            if ((_generalPurposeBitFlag & BitFlagValues.UnicodeFileName) == 0)
            {
                readEntryNameEncoding = (_archive == null)
                                            ...
                                            : _archive.EntryNameEncoding ?? Encoding.GetEncoding(0);
            }
            else
            {
        ...
        private Byte[] EncodeEntryName(String entryName, out bool isUTF8)
        {
            ...
            isUTF8 = (writeEntryNameEncoding is UTF8Encoding) && writeEntryNameEncoding.Equals(Encoding.UTF8);
 ...
 || _archive._forceZip64
        ...
        {
            if (_everOpenedForWrite)
                throw new IOException(System.IO.Compression.Messages.CreateModeWriteOnceAndOneEntryAtATime);
        ...
        {
            if (_currentlyOpenForWrite)
                throw new IOException(System.IO.Compression.Messages.UpdateModeOneStream);
            ...
            if (_originallyInArchive)
            {
                if (needToUncompress)
                {
                    if (CompressionMethod != CompressionMethodValues.Stored &&
                        CompressionMethod != CompressionMethodValues.Deflate)
                    {
                        message = System.IO.Compression.Messages.UnsupportedCompression;
                        return false;
                    }
                }
                if (_diskNumberStart != _archive.NumberOfThisDisk)
                {
                    message = System.IO.Compression.Messages.SplitSpanned;
                    return false;
                }
                if (_offsetOfLocalHeader > _archive.ArchiveStream.Length)
                {
                    message = System.IO.Compression.Messages.LocalFileHeaderCorrupt;
                    return false;
                }
                _archive.ArchiveStream.Seek(_offsetOfLocalHeader, SeekOrigin.Begin);
                if (!ZipLocalFileHeader.TrySkipBlock(_archive.ArchiveReader))
                ...
                if (needToLoadIntoMemory)
                {
                    ...
                    {
                        message = System.IO.Compression.Messages.EntryTooLarge;
                        return false;
                    }
                }
            }
...
;
        ...
        {
            if (_archive == null)
                throw new InvalidOperationException(System.IO.Compression.Messages.DeletedEntry);
            _archive.ThrowIfDisposed();
        }
        ...
        private class DirectToArchiveWriterStream : Stream
        {
            ...
            public override Int64 Length
            {
                ...
                {
                    ThrowIfDisposed();
                    throw new NotSupportedException(System.IO.Compression.Messages.SeekingNotSupported);
                }
            }
            ...
            {
                if (_isDisposed)
                    throw new ObjectDisposedException(this.GetType().Name, System.IO.Compression.Messages.HiddenStreamName);
            }
            ...
            {
                ThrowIfDisposed();
                throw new NotSupportedException(System.IO.Compression.Messages.SetLengthRequiresSeekingAndWriting);
            }
            ...
            public override void Write(Byte[] buffer, Int32 offset, Int32 count)
            {
                ...
                if (offset < 0)
                    throw new ArgumentOutOfRangeException("offset", System.IO.Compression.Messages.ArgumentNeedNonNegative);
                if (count < 0)
                    throw new ArgumentOutOfRangeException("count", System.IO.Compression.Messages.ArgumentNeedNonNegative);
                if ((buffer.Length - offset) < count)
                    throw new ArgumentException(System.IO.Compression.Messages.OffsetLengthInvalid);
