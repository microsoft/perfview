...
namespace Example
{
    class Program
    {
        [DLLImport("MyNativeLibrary.so")]
        public static extern void MyNativeFunction();

        static void A() => B();
        static void B() => C();
        static void C() => MyNativeFunction();
...
For the above example, this means that the "Thread Time" view will show frames that look like `Main->CPU_TIME` and `Main->A->B->C->MyNativeFunction->UNMANAGED_CODE_TIME`.

As a rule of thumb, the last frame before `CPU_TIME` frames is actively on the CPU and most likely has expensive logic in it. Similarly, the last frame before `UNMANAGED_CODE_TIME` is not what is actively doing work, but will give you a good indicator of what _is_ doing work during that sample.